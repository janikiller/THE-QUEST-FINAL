...
function drawCachedGround(ctx, game, phase, camX, camY, x0, y0, x1, y1) {
  ...
  if (!_groundCanvas || _groundKey !== baseKey || outOfBounds) {
    ...
    // Pass 1: rellenos CONTINUOS (solape grande → sin costuras cuadradas)
    for (let ty = cy0; ty < cy1; ty++) {
      for (let tx = cx0; tx < cx1; tx++) {
        const tile = game.world.tiles[ty * size + tx];
        if (isAnimatedGround(tile)) continue;
        const px = (tx - cx0) * TILE_PX;
        const py = (ty - cy0) * TILE_PX;
        paintTerrainBase(g, game, tile, tx, ty, px, py, phase);
      }
    }
    // Pass 2: detalle lineal (grietas, briznas, bordillos, líneas de carretera)
    for (let ty = cy0; ty < cy1; ty++) {
      for (let tx = cx0; tx < cx1; tx++) {
        const tile = game.world.tiles[ty * size + tx];
        if (isAnimatedGround(tile)) continue;
        const px = (tx - cx0) * TILE_PX;
        const py = (ty - cy0) * TILE_PX;
        paintTerrainDetail(g, game, tile, tx, ty, px, py, phase);
      }
    }
...
const GROUND_OVERLAP = 10; // solape amplio → costuras invisibles entre tiles

function paintTerrainBase(ctx, game, tile, tx, ty, px, py, phase) {
...
function paintTerrainDetail(ctx, game, tile, tx, ty, px, py, phase) {
  if (tile === TILE.ROAD || tile === TILE.CROSSWALK) {
    drawRoadDetail(ctx, game, tile, tx, ty, px, py);
    ...
    softTileEdges(ctx, game, tx, ty, px, py, tile);
  } else if (tile === TILE.ALLEY || tile === TILE.RUBBLE) {
    drawAlleyDetail(ctx, tile, tx, ty, px, py);
    softTileEdges(ctx, game, tx, ty, px, py, tile);
  } else if (tile === TILE.WALL && !(_viewIndoor && buildingAt(game.world, tx + 0.5, ty + 0.5) === _viewIndoor)) {
    // Grietas lineales ocasionales en fachada
    if (((tx * 5 + ty * 9) % 7) === 0) {
      ...
      ctx.quadraticCurveTo(px + 16, py + 20, px + 12, py + 40);
      ctx.stroke();
    }
  }
}
...
function drawGround(ctx, game, tile, tx, ty, px, py, phase) {
  // Usado solo para agua animada (fuera de caché) y fallback
  if (tile === TILE.WATER) {
    paintTerrainBase(ctx, game, tile, tx, ty, px, py, phase);
    drawWaterDetail(ctx, game, tx, ty, px, py, game.time);
    softTileEdges(ctx, game, tx, ty, px, py, tile);
    return;
  }
  paintTerrainBase(ctx, game, tile, tx, ty, px, py, phase);
  paintTerrainDetail(ctx, game, tile, tx, ty, px, py, phase);
}
...
function softTileEdges(ctx, game, tx, ty, px, py, selfTile) {
  ...
  for (const e of edges) {
    ...
    const roadish = (t) => t === TILE.ROAD || t === TILE.CROSSWALK || t === TILE.SIDEWALK;
    // Calles limpias: transición suave y corta entre asfalto/acera (sin manchas)
    const streetEdge = roadish(selfTile) && roadish(n);
    const depth = waterish ? 9 : grassish ? 8 : streetEdge ? 3.2 : 5.5;
    const seed = (tx * 17 + ty * 13 + e.dx * 3 + e.dy * 7) & 15;

    // Banda de relleno ondulada (mezcla visual, no línea recta)
    ctx.fillStyle = flatN;
    ctx.globalAlpha = waterish ? 0.28 : grassish ? 0.22 : streetEdge ? 0.08 : 0.14;
    ctx.beginPath();
    if (e.horiz) {
...
function roadRunsVertical(game, tx, ty) {
  const v = game.world.vRoads;
  if (v?.some((vx) => tx >= vx && tx < vx + 2)) return true;
  const h = game.world.hRoads;
  if (h?.some((hy) => ty >= hy && ty < hy + 2)) return false;
  const dir = game.world.crosswalkDir?.get(`${tx},${ty}`);
  if (dir === "v") return true;
  if (dir === "h") return false;
  const n = neighborTile(game, tx, ty - 1);
  const s = neighborTile(game, tx, ty + 1);
  const e = neighborTile(game, tx + 1, ty);
  const w = neighborTile(game, tx - 1, ty);
  const roadish = (t) => t === TILE.ROAD || t === TILE.CROSSWALK;
  return (roadish(n) ? 1 : 0) + (roadish(s) ? 1 : 0) >= (roadish(e) ? 1 : 0) + (roadish(w) ? 1 : 0);
}

/** Detalle de carretera limpia — líneas, cebras y bordillos (sin grietas ni manchas). */
function drawRoadDetail(ctx, game, tile, tx, ty, px, py) {
  ctx.lineCap = "round";
  ctx.lineJoin = "round";

  // Bordillo limpio hacia acera
  const inRota = game.world.roundabouts?.find((r) => Math.hypot(tx + 0.5 - r.x, ty + 0.5 - r.y) < r.r + 0.35);
  if (!inRota) {
    ctx.strokeStyle = "rgba(200,198,190,0.55)";
    ctx.lineWidth = 2.2;
    ctx.beginPath();
    ...
    if (neighborTile(game, tx - 1, ty) === TILE.SIDEWALK) {
    ...
    ctx.strokeStyle = "rgba(210,208,200,0.65)";
    ctx.lineWidth = 2.4;
    ctx.beginPath();
    ctx.arc(ox, oy, inRota.island * TILE_PX + 2, 0, Math.PI * 2);
    ctx.stroke();
    const gap = 0.32;
    ctx.strokeStyle = "rgba(200,198,190,0.5)";
    ctx.lineWidth = 2.6;
    ctx.lineCap = "butt";
    ctx.beginPath();
    for (let i = 0; i < 4; i++) {
  ...
  if (tile === TILE.CROSSWALK) {
    // Cebra limpia y bien alineada al ancho de la calzada
    ctx.fillStyle = "rgba(245,245,240,0.92)";
    if (vertRoad) {
      for (let i = 0; i < 5; i++) {
        const x = px + 6 + i * 8;
        fillRound(ctx, x, py + 4, 5, TILE_PX - 8, 1.5);
      }
    } else {
      for (let i = 0; i < 5; i++) {
        const y = py + 6 + i * 8;
        fillRound(ctx, px + 4, y, TILE_PX - 8, 5, 1.5);
      }
    }
  } else {
    const rota = game.world.roundabouts?.find((r) => Math.hypot(tx + 0.5 - r.x, ty + 0.5 - r.y) < r.r + 1.8);
    if (!rota) {
      ctx.strokeStyle = "rgba(235, 205, 70, 0.7)";
      ctx.lineWidth = 2.4;
      ...
      if (onApproach && d > rota.island + 0.2) {
        ...
        if (!inMouth) {
          ctx.strokeStyle = "rgba(235, 205, 70, 0.55)";
          ctx.lineWidth = 2.2;
          ctx.beginPath();
  ...
  if (up === TILE.WATER || down === TILE.WATER || neighborTile(game, tx - 1, ty) === TILE.WATER || neighborTile(game, tx + 1, ty) === TILE.WATER) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      ctx.beginPath();
      ctx.moveTo(px + 6, y);
      ctx.lineTo(px + TILE_PX - 6, y);
      ctx.stroke();
    }
  }
}

/** Acera limpia — bordillo claro solo hacia la calle. */
function drawSidewalkDetail(ctx, game, tx, ty, px, py) {
  ctx.lineCap = "round";

  const roadN = neighborTile(game, tx, ty - 1);
  const roadS = neighborTile(game, tx, ty + 1);
  const roadW = neighborTile(game, tx - 1, ty);
  const roadE = neighborTile(game, tx + 1, ty);
  const isRoadish = (t) => t === TILE.ROAD || t === TILE.CROSSWALK;
  ctx.fillStyle = "rgba(70,70,74,0.45)";
  ...
  if (rota) {
    ...
    const oy = (rota.y - ty) * TILE_PX + py;
    const gap = 0.32;
    ctx.strokeStyle = "rgba(70,70,74,0.55)";
    ctx.lineWidth = 3.2;
    ctx.lineCap = "butt";
    ctx.beginPath();
    for (let i = 0; i < 4; i++) {
    ...
    ctx.stroke();
    ctx.strokeStyle = "rgba(210,208,200,0.45)";
    ctx.lineWidth = 2;
    ctx.beginPath();
    for (let i = 0; i < 4; i++) {
      const mid = -Math.PI / 2 + i * (Math.PI / 2);
  ...
  if (isRoadish(roadN)) fillRound(ctx, px + 1, py + 1, TILE_PX - 2, 3.2, 1.2);
  if (isRoadish(roadW)) fillRound(ctx, px + 1, py + 1, 3.2, TILE_PX - 2, 1.2);
  if (isRoadish(roadE)) fillRound(ctx, px + TILE_PX - 4.2, py + 1, 3.2, TILE_PX - 2, 1.2);
}
...
function drawWaterDetail(ctx, game, tx, ty, px, py, time) {
  ...
  if (((tx * 5 + ty * 3) % 7) === 0) {
    ctx.fillStyle = "rgba(40, 90, 40, 0.28)";
    ctx.beginPath();
    ctx.ellipse(px + 22, py + 24, 14, 6, 0.3, 0, Math.PI * 2);
    ctx.fill();
  }
  if (((tx + ty * 2) % 11) === 0) {
    ctx.fillStyle = "rgba(20, 16, 10, 0.35)";
    ctx.beginPath();
    ctx.ellipse(px + 18, py + 16, 10, 4, -0.2, 0, Math.PI * 2);
    ctx.fill();
  }

  if (neighborTile(game, tx, ty - 1) !== TILE.WATER && neighborTile(game, tx, ty - 1) !== TILE.ROAD) {
    ctx.strokeStyle = "rgba(70,60,48,0.8)";
    ctx.lineWidth = 4.5;
    ctx.beginPath();
    ctx.moveTo(px - 2, py + 4);
    ctx.quadraticCurveTo(px + 18, py + 1, px + 28, py + 5);
    ctx.quadraticCurveTo(px + 40, py + 2, px + TILE_PX + 2, py + 4);
    ctx.stroke();
  }
  if (neighborTile(game, tx, ty + 1) !== TILE.WATER && neighborTile(game, tx, ty + 1) !== TILE.ROAD) {
    ctx.strokeStyle = "rgba(70,60,48,0.8)";
    ctx.lineWidth = 4.5;
    ctx.beginPath();
    ctx.moveTo(px - 2, py + TILE_PX - 4);
    ctx.quadraticCurveTo(px + 20, py + TILE_PX - 1, px + TILE_PX + 2, py + TILE_PX - 5);
    ctx.stroke();
  }
  ...
  ctx.lineWidth = 1.5;
  ctx.beginPath();
  ctx.moveTo(px - 1, py + 16 + wave);
  ctx.quadraticCurveTo(px + 22, py + 12 + wave, px + TILE_PX + 1, py + 18 + wave);
  ctx.stroke();
  ctx.strokeStyle = "rgba(60, 100, 70, 0.16)";
  ctx.beginPath();
  ctx.moveTo(px - 1, py + 30 + wave * 0.6);
  ctx.quadraticCurveTo(px + 24, py + 28 + wave * 0.6, px + TILE_PX + 1, py + 32 + wave * 0.6);
  ctx.stroke();

  if (((tx + ty) % 8) === 0) {
    ctx.fillStyle = "rgba(120, 140, 60, 0.3)";
    ctx.beginPath();
    ctx.ellipse(px + 20, py + 22 + wave, 5, 2.2, 0.1, 0, Math.PI * 2);
    ctx.fill();
  }
}
...
function drawGrassDetail(ctx, game, tx, ty, px, py) {
  ...
  if (((tx * 3 + ty * 5) % 5) === 0) {
    ctx.fillStyle = "rgba(40,78,36,0.16)";
    ctx.beginPath();
    ctx.ellipse(px + 18 + (tx % 8), py + 20 + (ty % 6), 11, 6, 0.2, 0, Math.PI * 2);
    ctx.fill();
  }
  if (((tx * 5 + ty * 3) % 17) === 0) {
    ctx.fillStyle = "rgba(96,74,44,0.22)";
    ctx.beginPath();
    ctx.ellipse(px + 24, py + 26, 11, 6, 0.25, 0, Math.PI * 2);
    ctx.fill();
  }
  ...
  for (let i = 0; i < 16; i++) {
    ...
    const gy = py + 4 + ((ty * 7 + i * 9) % 42);
    const lean = ((i % 5) - 2) * 2.4;
    const tall = 9 + (i % 5);
    ctx.strokeStyle = i % 3 === 0 ? "rgba(120,168,82,0.85)" : "rgba(52,102,50,0.65)";
    ctx.lineWidth = i % 4 === 0 ? 1.9 : 1.15;
    ctx.beginPath();
...
function drawAlleyDetail(ctx, tile, tx, ty, px, py) {
  ctx.lineCap = "round";
  if (((tx + ty) % 5) === 0) {
    ctx.fillStyle = "rgba(40,70,80,0.22)";
    ctx.beginPath();
    ctx.ellipse(px + 24, py + 30, 12, 5, 0, 0, Math.PI * 2);
    ctx.fill();
  }
  if (((tx * 3 + ty) % 7) === 0) {
    ...
    ctx.quadraticCurveTo(px + 14, py + 16, px + 22, py + 26);
    ctx.quadraticCurveTo(px + 16, py + 34, px + 8, py + 28);
    ctx.fill();
  }
  if (tile === TILE.RUBBLE && ((tx + ty * 2) % 4) === 0) {
    ctx.fillStyle = "rgba(110,100,88,0.5)";
    ctx.beginPath();
    ctx.ellipse(px + 18, py + 20, 8, 5, 0.3, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = "rgba(70,64,56,0.55)";
    ctx.beginPath();
    ctx.ellipse(px + 30, py + 28, 7, 3.5, -0.2, 0, Math.PI * 2);
    ctx.fill();
  }
}

// Compat wrappers (por si algún caller antiguo queda)
function drawRoad(ctx, game, tile, tx, ty, px, py, phase) {
  paintTerrainBase(ctx, game, tile, tx, ty, px, py, phase);
  drawRoadDetail(ctx, game, tile, tx, ty, px, py);
}
function drawSidewalk(ctx, game, tx, ty, px, py, phase) {
  paintTerrainBase(ctx, game, TILE.SIDEWALK, tx, ty, px, py, phase);
  drawSidewalkDetail(ctx, game, tx, ty, px, py);
}
function drawWater(ctx, game, tx, ty, px, py, time) {
  paintTerrainBase(ctx, game, TILE.WATER, tx, ty, px, py, null);
  drawWaterDetail(ctx, game, tx, ty, px, py, time);
}
...
function drawAlley(ctx, tile, tx, ty, px, py) {
  paintTerrainBase(ctx, null, tile, tx, ty, px, py, null);
  drawAlleyDetail(ctx, tile, tx, ty, px, py);
}
...
function drawInteriorFloor(ctx, tile, px, py, b, floorStyle, tx, ty) {
  if (tile === TILE.BASE) {
    ...
    ctx.fillRect(px - GROUND_OVERLAP, py - GROUND_OVERLAP, TILE_PX + GROUND_OVERLAP * 2, TILE_PX + GROUND_OVERLAP * 2);
    strokeRound(ctx, px + 6, py + 6, TILE_PX - 12, TILE_PX - 12, 4, "rgba(180, 210, 120, 0.45)", 1.5);
    return;
  }

  // Color plano por estilo (sin damero odd/even → sin grilla)
  if (floorStyle === "tile") {
    ctx.fillStyle = "#9a9084";
    ctx.fillRect(px - GROUND_OVERLAP, py - GROUND_OVERLAP, TILE_PX + GROUND_OVERLAP * 2, TILE_PX + GROUND_OVERLAP * 2);
    // Juntas suaves solo cada 2 tiles en X (líneas largas, no cruz por baldosa)
    if ((tx % 2) === 0) {
      ctx.strokeStyle = "rgba(48,40,34,0.18)";
      ctx.lineWidth = 1.1;
      ctx.lineCap = "round";
      ctx.beginPath();
      ctx.moveTo(px + TILE_PX, py - 1);
      ctx.lineTo(px + TILE_PX, py + TILE_PX + 1);
      ctx.stroke();
    }
    if ((ty % 2) === 0) {
      ctx.strokeStyle = "rgba(48,40,34,0.14)";
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(px - 1, py + TILE_PX);
      ctx.lineTo(px + TILE_PX + 1, py + TILE_PX);
      ctx.stroke();
    }
    ...
    if (((tx * 3 + ty) % 5) === 0) {
      ...
      ctx.quadraticCurveTo(px + 22, py + 18, px + 44, py + 28);
      ctx.stroke();
    }
  } else {
    // wood — tablones horizontales contínuos (mismo tono)
    ctx.fillStyle = "#826044";
    ctx.fillRect(px - GROUND_OVERLAP, py - GROUND_OVERLAP, TILE_PX + GROUND_OVERLAP * 2, TILE_PX + GROUND_OVERLAP * 2);
    ...
    ctx.lineTo(px + TILE_PX + 1, py + 13);
    ctx.moveTo(px - 1, py + 28);
    ctx.lineTo(px + TILE_PX + 1, py + 27);
    ctx.stroke();
  }
}
...
function drawFloorDecor(ctx, dec, px, py) {
  if (dec.kind === "rug") {
    ...
    fillRound(ctx, px + inset + 2, py + inset + 3, TILE_PX - inset * 2, TILE_PX - inset * 2, 5);
    ctx.fillStyle = c;
    fillRound(ctx, px + inset, py + inset, TILE_PX - inset * 2, TILE_PX - inset * 2, 5);
    strokeRound(ctx, px + inset + 1, py + inset + 1, TILE_PX - inset * 2 - 2, TILE_PX - inset * 2 - 2, 4, shade(c, 30), 1.5);
    strokeRound(ctx, px + inset + 4, py + inset + 4, TILE_PX - inset * 2 - 8, TILE_PX - inset * 2 - 8, 3, shade(c, -20), 1);
    ctx.fillStyle = shade(c, 22);
    ctx.globalAlpha = 0.45;
    if (dec.variant === 1) {
      fillRound(ctx, px + TILE_PX / 2 - 5, py + TILE_PX / 2 - 5, 10, 10, 3);
    } else {
      ctx.beginPath();
...
function drawBuildingRoof(ctx, b, camX, camY, phase, entered, litWindows = []) {
  ...
  if (style === "warehouse") {
    ...
    ctx.lineWidth = 1.6;
    for (let i = 0; i < 3; i++) {
      strokeRound(ctx, px + 6 + i * 4, py + 8, t - 14, t - 16, 3, "rgba(0,0,0,0.22)", 1.6);
      strokeRound(ctx, px + 6 + i * 4, py + bh - t + 8, t - 14, t - 16, 3, "rgba(0,0,0,0.22)", 1.6);
    }
  } else if (style === "shop") {
    ...
    fillRound(ctx, px + t + 4, py + bh - t + 14, bw - t * 2 - 8, t - 20, 4);
    strokeRound(
      ctx,
      px + t + 4,
      py + bh - t + 14,
      ...
      t - 20,
      4,
      shopLit ? "rgba(255, 220, 150, 0.35)" : "rgba(200,210,220,0.35)",
      1.2
    );
    if (bw > 70) {
      const neon = b.awning || "#8a3030";
      ctx.fillStyle = shopLit ? neon : shade(neon, -40);
      fillRound(ctx, px + t + 8, py + bh - t + 2, Math.min(120, bw - t * 2 - 16), 13, 4);
      if (shopLit) {
        litWindows.push({ x: px + t + 40, y: py + bh - t + 8 });
        ctx.fillStyle = "rgba(255, 180, 120, 0.12)";
        fillRound(ctx, px + t + 6, py + bh - t, Math.min(124, bw - t * 2 - 12), 16, 4);
      }
  ...
  if (!entered) {
    ...
    const roofH = bh - t * 2;
    if (roofW > 0 && roofH > 0) {
      // Tejado redondeado con pendiente visual
      ...
      ctx.fillStyle = roof;
      fillRound(ctx, roofX, roofY, roofW, roofH, 5);
      strokeRound(ctx, roofX, roofY, roofW, roofH, 5, "rgba(10,10,12,0.35)", 1.2);

      // Canalones / líneas de pendiente (suaves)
      ctx.strokeStyle = "rgba(0,0,0,0.16)";
      ctx.lineWidth = 1;
      ctx.lineCap = "round";
      for (let i = 1; i < 4; i++) {
        ctx.beginPath();
        ctx.moveTo(roofX + 6, roofY + (roofH / 4) * i);
        ctx.lineTo(roofX + roofW - 6, roofY + (roofH / 4) * i);
        ctx.stroke();
      }

      // Pocos lucernarios redondeados
      ...
      for (let row = 0; row < rows; row++) {
        for (let col = 0; col < cols; col++) {
          ...
          fillRound(ctx, wx, wy, ww, wh, 2);
          if (lit && (col + row) % 5 === 0) litWindows.push({ x: wx + ww / 2, y: wy + wh / 2 });
        }
      }

      // Equipos de azotea redondeados
      ctx.fillStyle = "#6a6e74";
      fillRound(ctx, roofX + roofW * 0.62, roofY + roofH * 0.18, 20, 14, 3);
      ctx.fillStyle = "#3a3e44";
      fillRound(ctx, roofX + roofW * 0.66, roofY + roofH * 0.1, 3, 14, 1.5);
      ...
      if (style === "tower") {
        ...
        fillRound(ctx, roofX + roofW * 0.38, roofY + roofH * 0.32, roofW * 0.24, roofH * 0.28, 4);
      }

      // Letrero
      if (roofW > 70) {
        ctx.fillStyle = "rgba(0,0,0,0.55)";
        fillRound(ctx, roofX + 6, roofY + 6, Math.min(150, roofW - 12), 20, 4);
...
function drawWindow(ctx, x, y, lit, style = "block") {
  ...
  strokeRound(ctx, x, y, w, h, 3, lit ? "rgba(255, 200, 120, 0.55)" : "rgba(0,0,0,0.4)", 1.1);
  if (style !== "warehouse") {
    ctx.strokeStyle = lit ? "rgba(255, 200, 120, 0.4)" : "rgba(0,0,0,0.35)";
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(x + w / 2, y + 2);
    ctx.lineTo(x + w / 2, y + h - 2);
