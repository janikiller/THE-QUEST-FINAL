...
func _ground_shot() -> void:
	...
	# García táctico: azul/navy policial (no Kick-Ass verde).
	if combat.player_sprite and combat.player_sprite.texture:
		...
		if himg:
			...
			for y in range(himg.get_height()):
				for x in range(himg.get_width()):
					...
					if c.g > c.b + 0.12 and c.g > c.r + 0.05 and lum > 0.22:
						greens += 1
					elif lum < 0.45 and (c.b >= c.g * 0.85):
						darks += 1
			print("HERO_COLORS green=", greens, " dark_tactical=", darks, " size=", himg.get_width(), "x", himg.get_height())
			if darks < 40 or greens > darks:
				print("GROUND_SHOT_FAIL hero still looks like Kick-Ass green (want García policial)")
				get_tree().quit(1)
				return
		else:
		...
		get_tree().quit(1)
		return
	await _save_shot("garcia_tactica_protagonista")
...
func _boss_fight_demo() -> void:
	...
	p.erase("_awaiting_combat")
	GameState.set_patrol(p)
	# Forzar duelo limpio por si la misión venía con escoltas viejas.
	if GameState.active_missions.has(boss_mid):
		var bm: Dictionary = GameState.active_missions[boss_mid]
		bm["solo_boss"] = true
		var only: Array = []
		for s in bm.get("suspects", []):
			if bool(s.get("is_boss", false)) or str(s.get("id", "")).begins_with("boss_"):
				only.append(s)
				break
		if only.is_empty() and not bm.get("suspects", []).is_empty():
			only.append(bm["suspects"][0])
		if not only.is_empty():
			bm["suspects"] = only
		GameState.active_missions[boss_mid] = bm
	$UI/UIRouter.begin_fight(boss_mid)
	await get_tree().process_frame
	await get_tree().process_frame
	await get_tree().create_timer(0.8).timeout
	var n_enemies := CombatState.enemies.size() if CombatState.is_active() else -1
	print("BOSS_FIGHT enemies=", n_enemies, " solo=", n_enemies == 1)
	if n_enemies != 1:
		print("BOSS_FIGHT_FAIL expected_solo got=", n_enemies)
	await _save_shot("boss_solo_idle")
	# Pelea justa jugable: defensa + ataques + turno del boss.
	if CombatState.is_active():
		var combat = $UI/UIRouter.get_node_or_null("CombatScreen")
		if combat and n_enemies == 1:
			...
			if CombatState.hand.size() >= 2:
				...
				if CombatState.hand.size() >= 3:
					CombatState.hand[2] = "codazo"
				combat._refresh()
			await get_tree().create_timer(0.7).timeout
			if CombatState.can_play_card("bloqueo"):
				print("BOSS_SOLO_DEMO guard")
				combat._busy = true
			...
			if CombatState.can_play_card("punetazo"):
				...
				combat._refresh()
				await get_tree().create_timer(0.55).timeout
				await _save_shot("boss_solo_after_hit")
			if CombatState.can_play_card("codazo"):
				print("BOSS_SOLO_DEMO elbow")
			...
			if combat.has_method("_on_end_turn"):
				print("BOSS_SOLO_DEMO end_turn")
				await combat._on_end_turn()
			await get_tree().create_timer(1.6).timeout
			await _save_shot("boss_solo_enemy_turn")
	print("BOSS_FIGHT_OK solo=", n_enemies == 1)
...
func _arrest_finale_demo() -> void:
	## Evidencia: victoria → esposas → DETENIDO → banner DETENCIÓN COMPLETADA.
	await get_tree().create_timer(0.8).timeout
	...
	# Un golpe visible y luego cierre forzado para enseñar la cinemática completa.
	if CombatState.hand.size() > 0:
		...
		if CombatState.can_play_card("punetazo"):
			...
			combat._refresh()
			await get_tree().create_timer(0.45).timeout
	# Derriba y dispara victoria → _play_arrest_finale
	...
	# Espera esposas / banner / panel de resultado.
	var saw_cuffs := false
	...
	for _t in range(90):
		...
		if combat == null:
			continue
		var cuffs := combat.find_child("ArrestCuffs", true, false)
		var banner := combat.find_child("ArrestBanner", true, false)
		if cuffs and not saw_cuffs:
			saw_cuffs = true
			await _save_shot("arrest_cuffs")
			print("ARREST_FINALE saw_cuffs")
		if banner and not saw_banner:
			saw_banner = true
			await _save_shot("arrest_banner")
			print("ARREST_FINALE saw_banner")
		if combat.result_panel and combat.result_panel.visible and not saw_result:
			...
			break
		if saw_cuffs and saw_banner and saw_result:
			break
	print(
		"ARREST_FINALE_OK cuffs=", saw_cuffs,
		" banner=", saw_banner,
		" result=", saw_result
	)
	await get_tree().create_timer(0.6).timeout
	if OS.get_environment("TQ_ARREST_FINALE_QUIT") != "0":
		get_tree().quit(0 if (saw_cuffs or saw_banner or saw_result) else 1)
